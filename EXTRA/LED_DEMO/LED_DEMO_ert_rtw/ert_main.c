...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(LED_DEMO_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    LED_DEMO_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(LED_DEMO_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(LED_DEMO_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(LED_DEMO_M, 0);
